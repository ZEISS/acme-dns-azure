...
def load(config_yaml: str = ''):
    try:
        ...
        raise ConfigurationError('Unable to parse configuration: %s' % e)
    ...
    if config['account_secret_name'] == '':
        config['account_secret_name'] = 'acme-account-%s' % re.sub('[^-a-zA-Z0-9]+', '-', config['server'])
...
def load_from_base64_env_var(env_var: str = None):
    try:
        ...
        if env_config_b64:
            ...
            raise ConfigurationError("Environment variable '%s' has an empty value." % env_var)
    except base64.binascii.Error as e:
        raise ConfigurationError("Unable to base64 decode configuration provided in environment variable '%s': %s" % (env_var, e))
    except Exception as e:
...
def load_from_file(filename: str = None):
    try:
        ...
        raise ConfigurationError("Unable to read configuration from '%s': %s" % (filename, e.strerror))
    except Exception as e:
        raise ConfigurationError('Error while loading configuration: %s' % e)
